...
from .buildingblocks_bf import DoubleConv, ResNetBlock, ResNetBlockSE, create_decoders, create_encoders
from .utils_bf import get_class, number_of_features_per_level
...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
                 ...
                 positional_encoding=False, use_bias=True,pool_type = 'max', padding_mode='zeros'):
        ...
        self.encoders = create_encoders(in_channels, f_maps, basic_module, conv_kernel_size,
                                        ...
                                        is3d, positional_encoding,pool_type=pool_type,
        ...
        self.decoders = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding,
                                        ...
                                        is3d, padding_mode=padding_mode)

        # in the last layer a 1×1 convolution reduces the number of output channels to the number of labels
        if is3d:
            self.final_conv = nn.Conv3d(f_maps[0], out_channels, 1, bias=use_bias, padding_mode=padding_mode)
        else:
            self.final_conv = nn.Conv2d(f_maps[0], out_channels, 1, bias=use_bias, padding_mode=padding_mode)
...
class UNet3D(AbstractUNet):
    ...
    def __init__(self, in_channels=1, out_channels=1, final_sigmoid=False, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=4, is_segmentation=False, conv_padding=1,
                 conv_upscale=2, upsample='default', dropout_prob=0.1,positional_encoding  =False, 
                 pool_type = 'max',use_bias = True, padding_mode='zeros', **kwargs):
