...
from .se_bf import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
import math
...
class SinusoidalPositionEmbeddings(nn.Module):
    ...
    def forward(self, t):
        half_dim = self.dim // 2
        emb = torch.exp(-torch.arange(half_dim, dtype=torch.float32, device = t.device) * (math.log(10000.0) / half_dim))
        emb = t[:, None] * emb[None, :]
        return torch.cat((torch.sin(emb), torch.cos(emb)), dim=-1)

def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding,
                dropout_prob, is3d,use_bias, padding_mode):
    """
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            if use_bias is False:
                bias = False
            if is3d:
                conv = nn.Conv3d(in_channels, out_channels, kernel_size, padding=padding, bias=bias, padding_mode=padding_mode)
            else:
                conv = nn.Conv2d(in_channels, out_channels, kernel_size, padding=padding, bias=bias, padding_mode=padding_mode)
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='gcr', num_groups=8,
                 padding=1, dropout_prob=0.1, is3d=True, use_bias = False, padding_mode='zeros' ):
        super(SingleConv, self).__init__()

        for name, module in create_conv(in_channels, out_channels, kernel_size, order,
                                        num_groups, padding, dropout_prob, is3d, use_bias, padding_mode=padding_mode):
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='gcr',
                 num_groups=8, padding=1, upscale=2, dropout_prob=0.1, is3d=True,use_bias = False, padding_mode='zeros'):
        ...
        self.add_module('SingleConv1',
                        SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, order, num_groups,
                                   padding=padding, dropout_prob=dropout_prob1, is3d=is3d, use_bias=use_bias,padding_mode=padding_mode))
        # conv2
        self.add_module('SingleConv2',
                        SingleConv(conv2_in_channels, conv2_out_channels, kernel_size, order, num_groups,
                                   padding=padding, dropout_prob=dropout_prob2, is3d=is3d, use_bias=use_bias, padding_mode=padding_mode))
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 ...
                 num_groups=8, padding=1, upscale=2, dropout_prob=0.1, is3d=True,positional_encoding=False,
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=2, basic_module=DoubleConv,
                 ...
                 dropout_prob=0.1, is3d=True,use_bias=False, padding_mode='zeros'):
        ...
        self.basic_module = basic_module(in_channels, out_channels,
                                         ...
                                         padding=padding,
                                         dropout_prob=dropout_prob,
                                         is3d=is3d,padding_mode=padding_mode,)
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding,
                    ...
                    layer_order, num_groups, pool_kernel_size, is3d, positional_encoding,
                    pool_type  = 'max',use_bias=False , padding_mode='zeros'):
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order,
                    num_groups, upsample, dropout_prob, is3d,use_bias=False, padding_mode='zeros'):
...
class TransposeConvUpsampling(AbstractUpsampling):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, scale_factor=2, is3d=True, use_bias=False):
        # make sure that the output size reverses the MaxPool3d from the corresponding encoder
        if is3d is True:
            conv_transposed = nn.ConvTranspose3d(in_channels, out_channels, kernel_size=kernel_size,
                                                 stride=scale_factor, padding=1, bias=use_bias)
        else:
